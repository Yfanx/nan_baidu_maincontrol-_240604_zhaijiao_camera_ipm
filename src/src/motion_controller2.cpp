...
public:
  ...
  struct Params {
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(Params, speedLow, speedHigh, speedDown,
                                   speedBridge, speedring, speedGarage,
                                   runP1, runP2, runP3, turnP, turnD, debug,
                                   saveImage, rowCutUp, rowCutBottom,
                                   validRowscenter, disGarageEntry,
                                   GarageEnable, BridgeEnable, FreezoneEnable,
                                   RingEnable, CrossEnable, GranaryEnable,
                                   DepotEnable, FarmlandEnable, SlowzoneEnable,
                                   circles, pathVideo); // 添加构造函数
  };
...
pathstype pathstype ; 
  ...
  void pdController(int controlCenter, double jiaodu, Edgeipm &edgeipm) {
   // float error = 160 - controlCenter; // 图像控制中心转换偏差    ---左  负数
      float error=160-controlCenter; // 图像控制中心转换偏差    ---左  负数 ----右  正数                                        // 要写入的文件路径
 ...
 //float error =jiaodu;   // 角度作为控制量，波动很大，问题严重图像控制中心转换偏差    ---左  负数
   // if (abs(error - errorLast) > 32) {
    //   error = error > errorLast ? errorLast + 32 : errorLast - 32;
    // }    
...
*/  //abs(jiaodu) * 
    //params.turnP = params.runP2 + params.runP1;
  ...
  void speedController(bool enable, bool slowDown, ControlCenterCal control) {
    ...
    ////////////////////
    // if (control.trackstdev < 100)
    // {
    //   if (control.tracklength > 210)
    //   {
    //     motorSpeed = params.speedHigh; // 2.5m
    //   }
    //   else if (control.tracklength > 190) // 加速使能
    //   {
    //     motorSpeed = params.speedHigh - 0.5;
    //   }
    // }

    // else if (control.tracklength > 120)
    //   motorSpeed = params.speedHigh - 1.5;

    // else
    //   motorSpeed = params.speedHigh - 2;
    ...
    //         return;
    //     }
    //     if (control.centerEdge[control.centerEdge.size() - 1].x > ROWSIMAGE /
    //     2)
    //     {
    //         motorSpeed = params.speedLow;
    //         counterShift = controlLow;
