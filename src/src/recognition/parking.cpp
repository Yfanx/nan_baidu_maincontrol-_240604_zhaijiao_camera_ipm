...
public:
   ...
   * @return false
   */
  bool process(TrackRecognition &
          trackRecognition) // trackRecognition.garageEnable.x,trackRecognition.garageEnable.y
  {
    switch (step) {
    case Step::init: // 初始化：起点斑马线屏蔽
      countSes++;

      if (trackRecognition.garageEnable.x == 1) // AI识别标志
      {
        if (trackRecognition.garageEnable.y > ROWSIMAGE * 0.6) // 标志距离计算
        {
          countSes = 0;
          break;
        }
    ...
    case Step::det: // AI未识别
      ...
      if (trackRecognition.garageEnable.x == 1) // AI识别标志// AI识别标志
      {
        if (trackRecognition.garageEnable.y > ROWSIMAGE * 0.4) // 标志距离计算
        {
          countRec++;
          break;
        }
    ...
    case Step::enable: // 场景使能: 检测斑马线标识丢失
      ...
      if (trackRecognition.garageEnable.x == 1) // AI识别标志// AI识别标志
      {
        if (trackRecognition.garageEnable.y > ROWSIMAGE * 0.3) // 标志距离计算
        {
          countSes = 0;
          break;
        }
