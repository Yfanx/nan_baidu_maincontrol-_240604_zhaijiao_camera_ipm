...
public:
   Mat imageoriginal;
    ...
    vector<POINT> ipmpointsEdgeLeft;     // 赛道左边缘点集
    vector<POINT> ipmpointsEdgeRight;    // 赛道右边缘点集
    ...
    int validRowsLeftend = 0;            // 边缘有效行数（左）
    int validRowsRightend = 0;           // 边缘有效行数（右）
    POINT garageEnable = POINT(0, 0); // 车库识别标志：（x=1/0，y=row)
    uint16_t rowCutUp = 10;           // 图像顶部切行
    uint16_t rowCutBottom =0;       // 图像底部切行
  ...
  for (int i = validRowsLeft; i < validRowsLeftend; i++)
        {
            circle(trackImage, Point(pointsEdgeLeft[i].y, pointsEdgeLeft[i].x), 1,
                   Scalar(0, 255, 0), 5); // 绿色点
        }


    for (int i =validRowsRight; i <validRowsRightend; i++)
        {
            circle(trackImage, Point(pointsEdgeRight[i].y, pointsEdgeRight[i].x), 1,
                   Scalar(0, 255, 255),5); // 黄色点
        }
       ...
       string str = "Edge: " + formatDoble2String(stdevEdgeCal2(pointsEdgeLeft, validRowsLeft,pointsEdgeLeft.size()-1),1) + " ------- " + formatDoble2String(stdevEdgeCal2(pointsEdgeRight, validRowsRight,pointsEdgeRight.size()-1),1);
        putText(trackImage, str, Point(20,ROWSIMAGE - 100), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255), 1); // 斜率：左|右
    ...
    double stdevEdgeCal(vector<POINT> &v_edge, int img_height)
    {
        ...
        vector<int> v_slope;
        int step =10; // int step = 10;  v_edge.size()/10;
    ...
    double stdevEdgeCal2(vector<POINT> &v_edge, int startl,int endl)
    {
        if (v_edge.size() <2)
        {
            return 1000;
        ...
        for (int i =startl; i <endl; i ++)
        {
           ...
           v_slope.push_back((v_edge[i].slope* 100 ));
        }
        if (v_slope.size() > 1)
        {
            double sum = accumulate(begin(v_slope), end(v_slope), 0.0);
     ...
     *
     */
     //  --sample 125.jpg,时，计算有问题，需修正  240205
  int* validRowsCal(void)
    {

//        for (int i = 0; i < pointsEdgeLeft.size(); i++)
//             {
// printf("-1-track- pointsEdgeLeft[i].x =--%d-----%d\n", pointsEdgeLeft[i].x,pointsEdgeLeft[i].y);

//             }

  //printf("-1-concel--pointsEdgeRight.back().x---pointsEdgeRight.size() =--%d-----%d\n", pointsEdgeRight.back().x,pointsEdgeRight.size() );
        validRowsLeft = pointsEdgeLeft.size() - 1;   // 边缘有效行数（左）
        validRowsRight = pointsEdgeRight.size() - 1; // 边缘有效行数（右）

        validRowsLeftend = pointsEdgeLeft.size() - 1;
        validRowsRightend = pointsEdgeRight.size() - 1;
//validRowsLeft = 0;
        int logo = 0, d =2, cnt = 0;

        if (pointsEdgeLeft.size() > 10)
        {  
            for (int i = 5; i < pointsEdgeLeft.size(); i++)
            {
                if (pointsEdgeLeft[5].y < d && pointsEdgeLeft[i].y < d && pointsEdgeLeft[i - 1].y < d)
                {
                    cnt = cnt + 1;
                    // break;
                }
                ...
                if (cnt < 1)// &&i+5< pointsEdgeLeft.size()<pointsEdgeLeft[i+4].y>d && pointsEdgeLeft[i+5].y>d) //&&i<pointsEdgeLeft.size()*0.9
                {
            ...
            printf("-1-validcal-validRowsLeft----%d\n",validRowsLeft);
///////////////////////////////////////// 删除离散点--------


//    if (validRowsLeft<pointsEdgeLeft.size()-5)
//         {
//             for (int i =validRowsLeft; i < pointsEdgeLeft.size(); i++)
//             {
//                 if (abs(pointsEdgeLeft[i].y-pointsEdgeLeft[i-2].y)>10)
//                    validRowsLeft = validRowsLeft + 1;
//                 else 
//                     break;
...
/////////////////////////////////////////-------删除离散点--------
            cnt = 0;
            for (int i = pointsEdgeLeft.size() - 1; i > 5; i--)
            {
                if (pointsEdgeLeft[pointsEdgeLeft.size() - 1].y < d && pointsEdgeLeft[i].y < d && pointsEdgeLeft[i - 1].y < d)
                {
                    cnt = cnt + 1;
                    // break;
                }
                    ...
                    cnt = 0;
                if (cnt < 1) //&&i<pointsEdgeLeft.size()*0.9
                {
                    validRowsLeftend = i - 1;
  ...
  cnt = 0;
        ...
        {
            for (int i = 5; i < pointsEdgeRight.size(); i++)
            {
                if (pointsEdgeRight[5].y>COLSIMAGE-d && pointsEdgeRight[i].y>COLSIMAGE - d && pointsEdgeRight[i - 1].y>COLSIMAGE - d)
                {
                    cnt = cnt + 1;
                }
                else
                ...
                if (cnt < 1 &&i+4< pointsEdgeRight.size()&&pointsEdgeRight[i+3].y<COLSIMAGE-d&&pointsEdgeRight[i+4].y<COLSIMAGE - d) //&&i<pointsEdgeRight.size()*0.9
                {
                    validRowsRight = i;
                    break;
                }
            ...
            for (int i = pointsEdgeRight.size() - 1; i > 5; i--)
            {
                if (pointsEdgeRight[pointsEdgeRight.size() - 1].y > COLSIMAGE - d && pointsEdgeRight[i].y > COLSIMAGE - d && pointsEdgeRight[i - 1].y > COLSIMAGE - d)
                {
                    cnt = cnt + 1;
                }
                else
          ...
          int *valid_left_right= new int[4];
            
         valid_left_right[0]=validRowsLeft;
           valid_left_right[1]=validRowsRight;
           valid_left_right[2]= validRowsLeftend;
           valid_left_right[3]=  validRowsRightend;
          return valid_left_right;
    }
...
///printf("\n\n\n  lvbo left 0 \t\t %d\n", pointsEdgeLeft.size());
// 左边有效行
      int  i0=0,cnt=0,cnt2= 0;
        if (pointsEdgeLeft.size() >100)
        {
            for (int i = pointsEdgeLeft.size() - 1; i >= 1; i--)
            {
                if (pointsEdgeLeft[pointsEdgeLeft.size() - 1].y < 2 && pointsEdgeLeft[i].y < 2 && pointsEdgeLeft[i - 1].y < 2)
                {
                    cnt++;
                    cnt2++;
                }
                    ...
                    cnt = 0;
                if (cnt2 > 10 && cnt < 1&&pointsEdgeLeft[i - 1].x<120&&(pointsEdgeLeft[i - 1].y-pointsEdgeLeft[i].y )>150)
                {
                  pointsEdgeLeft.resize(i-2);
    ...
    //         printf("LEFT lvbo=%d=\t= %d=\t%d\t \n", i, pointsEdgeLeft[i].x, pointsEdgeLeft[i].y);
    //     }


 //            pointsEdgeRight.resize(validRowsRight);
     // 右边有效行
        // validRowsRight = 0;
...
void TrackRecognition::trackRecognition(bool isResearch, uint16_t rowStart) {
    ...
    //if (rowCutUp > ROWSIMAGE /6)
    //
    //     rowCutUp =10;
    // if (rowCutBottom > ROWSIMAGE /6)
    //     rowCutBottom = ROWSIMAGE /6;
    ...
    for (int row = rowStart; row > rowCutUp; row--) // 有效行：10~220
    {
        ...
        if (flagStartBlock)                            // 起始行做特殊处理
        {
            ...
            int limitWidthBlock = COLSIMAGE * 0.5; // 首行色块宽度限制（不能太小）
            if (row < ROWSIMAGE * 0.4)
            {
    ...
    validRowsCal(); // 有效行计算
// pointsEdgeLeft.resize(validRowsLeft);
//            pointsEdgeRight.resize(validRowsRight);
//printf(" track =validRowsLeft  right =%d---%d \n\n ",validRowsLeft ,validRowsRight );
//cout<<"validRowsRight="<<validRowsRight <<endl;
//printf(" --pointsEdgeLeft---Right.size()  =%d---%d \n\n ",pointsEdgeLeft.size() ,pointsEdgeRight.size() );
    //----240117
//
