...
//       printf("-41-concel-length--validRowsLeft=r--%d-- %d  ---%d\n" ,track.pointsEdgeLeft.size(), track.validRowsLeft, track.validRowsLeftend);

//  printf("-4-con track.pointsEdgeLeft.size()  y=%d\n",track.pointsEdgeLeft.size());
            track = retrack(track);
//                   printf("-42-concel-length--validRowsLeft=r--%d-- %d  ---%d\n" ,track.pointsEdgeLeft.size(), track.validRowsLeft, track.validRowsLeftend);

//  printf("\n\n-41-con track.pointsEdgeLeft.size()  y=%d\n",track.pointsEdgeLeft.size());
// for (int i = 0; i < track.pointsEdgeLeft.size(); i++)
//             {
//                 printf("-1-con i=%d --track.pointsEdgeLeft[i].x=%d  \t  y=%d\n", i, track.pointsEdgeLeft[i].x, track.pointsEdgeLeft[i].y); // 绿色点
//             }
...
//                       }
 ...
 {   
  ...
  for (int i = 0; i < centerEdgeipm.size(); i++)
        {
            circle(centerImage, Point( centerEdgeipm[i].y, centerEdgeipm[i].x), 5, Scalar(0, 0, 255), 1);
        }
