...
public:
  ...
  bool depotDetection(TrackRecognition &track, vector<PredictResult> predict) {
    ...
    case DepotStep::DepotNone: //[01] 维修厂标志检测
      if (counterImmunity > 20) {
        for (int i = 0; i < predict.size(); i++) {
          ...
          {
            counterRec++;
            cout<<"counterRec="<<counterRec<<endl;
            break;
          }
        }
        if (counterRec) {
    ...
    {
      counterExit++;
      //cout<<"counterExit"<<counterExit<<endl;
      if (counterExit > 300) {
        reset();
        return false;
      }
      ...
      {
        //cout<< "!!!!-cone-row1==" << "\t"  << "!!!!-.x y==" <<_pointNearCone.x<< "\t" <<_pointNearCone.y<<endl;
                // //
                  // cout<<"\n\n pointEdgeDet.size="<<pointEdgeDet.size()<<endl;
                  //                     for (int row1 =0; row1<pointEdgeDet.size(); ++row1)
                  //     {
                  //         cout<< "!!!!-cone-row1==" <<row1<< "\t"  << "!!!!-.x y==" <<pointEdgeDet[row1].x<< "\t" <<pointEdgeDet[row1].y<<endl;
        
                  //     }
                 
      }
     // 105 < _pointNearCone.x&& _pointNearCone.x< 125 && 75 <_pointNearCone.y && _pointNearCone.y < 90
      if (_pointNearCone.x > 100 && _pointNearCone.x <110 ) // 当车辆开始靠近右边锥桶：准备入库
      {
        counterRec++;
        cout<<"counterRec="<<counterRec<<endl;
        if (counterRec >= 8) {  //直道0.2速8  弯道0.2速   
    ...
    case DepotStep::DepotCruise: //[04] 巡航使能
    {
     if(DepotStep::DepotCruise)
     {
      ...
      //cout<<"lastPointsEdgeLeft="<<lastPointsEdgeLeft.size()<<endl;
      //cout<<"lastPointsEdgeRight="<<lastPointsEdgeRight.size()<<endl;
      //      if (lastPointsEdgeLeft.size() < ROWSIMAGE / 2 && lastPointsEdgeRight.size() < ROWSIMAGE / 2) 
      //      {
      //        counterExit++;
      //        //cout<<"counterExit="<<counterExit<<endl;
      //        if (counterExit > 0) 
      ...
      //       cout<<"heighestCone.y="<<heighestCone.y<<endl;
      //       if (heighestCone.y >= 90) // 顶角锥桶靠近右边→继续右转
      //       {
      //         indexDebug = 3;
      //         depotStep = DepotStep::DepotStop;          
      //     // vector<POINT> points =
      ...
      //     //       predictEdgeRight(track.pointsEdgeLeft); // 由左边缘补偿右边缘
...
private:
  ...
  POINT searchNearestCone(vector<POINT> pointsEdgeLeft,
    ...
    POINT point(0, 0);
    double disMin = 40; // 右边缘锥桶离赛道左边缘最小距离
  ...
  void searchHighestCone(vector<POINT> pointsCone) {
    ...
    for (int i = 0; i < pointsCone.size(); i++) {
      if (pointsCone[i].x < heighestCone.x)
        ...
        cout<< "!!!!-cone-row1==" << "\t"  << "!!!!-heighestCone.x y==" <<heighestCone.x<< "\t" <<heighestCone.y<<endl;
    }
  }
