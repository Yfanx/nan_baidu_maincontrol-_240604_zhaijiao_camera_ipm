...
public:
    ...
    {  
        ...
        case FarmlandStep::Enter: //[02] 开进进入
        {
            ...
            if (conesEdgeLeft.size() >= conesEdgeRight.size() && conesEdgeLeft.size() >= 1) // 左边补右边
            {
                vector<POINT> pointsRep;
               // cout<<"conesEdgeLeft.size="<<conesEdgeLeft.size()<<endl;
                //cout<<"conesEdgeRight.size="<<conesEdgeRight.size()<<endl;
              //  cout<<"conesEdgeRight[0].x="<<conesEdgeRight[0].x<<endl;
               ...
               // cout<<"indexDebug="<<indexDebug<<endl;
                conesEdgeRight.clear();
                ...
                vector<POINT> repair = Bezier(0.01, input);
                conesEdgeLeft.clear();
                conesEdgeLeft = repair;
            }
            else if (conesEdgeLeft.size() < conesEdgeRight.size() && conesEdgeRight.size() >= 1) // 右边补左边
            {
                // cout<<"\n\n conesEdgeLeft.size="<<conesEdgeLeft.size()<<endl;
                //  cout<<"\n\nconesEdgeRight.size="<<conesEdgeRight.size()<<endl;
                vector<POINT> pointsRep;
                if (conesEdgeLeft.size() > 0 && conesEdgeLeft[0].x > ROWSIMAGE / 3)
                {
                    pointsRep = predictEdgeLeft(conesEdgeRight, conesEdgeLeft[0], conesEdgeRight[0]);
                    indexDebug = 3;
                }
                else
                {
                    pointsRep = predictEdgeLeft(conesEdgeRight, POINT(0, 0), conesEdgeRight[0]);
                    indexDebug = 4;
                }
                conesEdgeLeft.clear();
...
//    /////////////删除候选点，参考历程
//         //
//                     for (int row1 =0; row1<conesEdgeLeft.size()-1; ++row1)
//                     {
//                         cout<< "!!!!-cone-row1==" <<row1<< "\t"  << "!!!!-.x y==" <<conesEdgeLeft[row1].x<< "\t" <<conesEdgeLeft[row1].y<<endl;
        
//                     }
//                  cout<<"\n\nconesEdgeRight.size="<<conesEdgeRight.size()<<endl;
//                     for (int row1 =0; row1<conesEdgeRight.size()-1; ++row1)
//                     {
//                         cout<< "!!!!-cone-row1==" <<row1<< "\t"  << "!!!!-.x y==" <<conesEdgeRight[row1].x<< "\t" <<conesEdgeRight[row1].y<<endl;
...
private:
    ...
    void searchConesEdge(vector<POINT> cones)
    {
        ...
        while (pointsCones.size() > 0)
        {
            ...
            {
                // //
                // cout<<"\n\n pointsCones.size="<<pointsCones.size()<<endl;
                //                     for (int row1 =0; row1<pointsCones.size()-1; ++row1)
                //     {
                //         cout<< "!!!!-cone-row1==" <<row1<< "\t"  << "!!!!-.x y==" <<pointsCones[row1].x<< "\t" <<pointsCones[row1].y<<endl;
        
                //     }
                // //
                if (conesEdgeLeft.size() > 0)
                {
                    if (distanceForPoints(conesEdgeLeft[conesEdgeLeft.size() - 1], pointsCones[i]) < conesEdgeLeft[conesEdgeLeft.size() - 1].x / 1.5 && pointsCones[i].x < conesEdgeLeft[conesEdgeLeft.size() - 1].x)
                    {
                        // double d = distanceForPoints(conesEdgeLeft[conesEdgeLeft.size() - 1], pointsCones[i]);
                        // cout<<"distance="<<d<<endl;
                        // cout<<"\n\n conesEdgeLeft[conesEdgeLeft.size() - 1].x / 1.5="<<conesEdgeLeft[conesEdgeLeft.size() - 1].x / 1.5<<endl;
                        // cout<<"\n\n pointsCones[i].x="<<pointsCones[i].x<<endl;
                        if (index < 0)
                            index = i;
            ...
            if (index >= 0)
            {
                ...
                int n = conesEdgeLeft.size();
                if(n > 6){
                    conesEdgeLeft.erase(conesEdgeLeft.begin() + 6,conesEdgeLeft.end());
                }
                int b = conesEdgeRight.size();
                if(b > 2){
                    conesEdgeRight.erase(conesEdgeRight.begin() + 2,conesEdgeRight.end());
                }    
                //    cout<<"\n\n conesEdgeLeft.size="<<conesEdgeLeft.size()<<endl;
...
//     /////////////删除候选点，参考历程
//          //
//                       for (int row1 =0; row1<conesEdgeLeft.size(); ++row1)
//                       {
//                           cout<< "!!!!-cone-row1==" <<row1<< "\t"  << "!!!!-.x y==" <<conesEdgeLeft[row1].x<< "\t" <<conesEdgeLeft[row1].y<<endl;
...
// // //           cout<< "!!!!track.widthBlock.size()="<<"\t" <<track.widthBlock.size()<<"\t" <<track.pointsEdgeLeft.size()<<endl;
//    cout<<"\n\nconesEdgeRight.size="<<conesEdgeRight.size()<<endl;
//                       for (int row1 =0; row1<conesEdgeRight.size(); ++row1)
//                       {
//                           cout<< "!!!!-cone-row1==" <<row1<< "\t"  << "!!!!-.x y==" <<conesEdgeRight[row1].x<< "\t" <<conesEdgeRight[row1].y<<endl;
...
//         ///////////////////////////---------------------------------------------------------------------------------------------------- [1] 入环判断
            ...
            for (int i = 0; i < pointsCones.size(); i++) // 搜索右边缘
            {
                ...
                {
                    if (distanceForPoints(conesEdgeRight[conesEdgeRight.size() - 1], pointsCones[i]) < conesEdgeRight[conesEdgeRight.size() - 1].x / 1.5 && pointsCones[i].x < conesEdgeRight[conesEdgeRight.size() - 1].x)
                    {
                        // double d = distanceForPoints(conesEdgeLeft[conesEdgeLeft.size() - 1], pointsCones[i]);
                        // cout<<"distance="<<d<<endl;
                        // cout<<"\n\n conesEdgeLeft[conesEdgeLeft.size() - 1].x / 1.5="<<conesEdgeLeft[conesEdgeLeft.size() - 1].x / 1.5<<endl;
...
//                     }
     ...
     * @return vector<POINT>
     */
    vector<POINT> predictEdgeRight(vector<POINT> &conesEdgeLeft, POINT pointL, POINT pointR)
    {
        int offset = 120; // 右边缘平移尺度
        vector<POINT> conesEdgeRight;
        POINT startPoint(0, 0);
        POINT endPoint(0, 0);
        Point2d prefictRight;
        if (conesEdgeLeft.size() < 3)
            return conesEdgeRight;
        ...
        {
            Point2d startIpm = ipm.homography(Point2d(conesEdgeLeft[0].y, conesEdgeLeft[0].x)); // 透视变换
        ...
        // Middle
        Point2d middleIpm = ipm.homography(Point2d(conesEdgeLeft[conesEdgeLeft.size() / 2].y, conesEdgeLeft[conesEdgeLeft.size() / 2].x)); // 透视变换
        offset =60;
        ...
        // End
        Point2d endIpm = ipm.homography(Point2d(conesEdgeLeft[conesEdgeLeft.size() - 1].y, conesEdgeLeft[conesEdgeLeft.size() - 1].x)); // 透视变换
        offset = 60;
        ...
        for (int i = 0; i < repair.size(); i++)
        {
            ...
            conesEdgeRight.push_back(repair[i]);
        }
     ...
     * @return vector<POINT>
     */
    vector<POINT> predictEdgeLeft(vector<POINT> &conesEdgeRight, POINT pointL, POINT pointR)
    {
        int offset = 120; // 右边缘平移尺度
        vector<POINT> conesEdgeLeft;
        POINT startPoint(0, 0);
        POINT endPoint(0, 0);
        Point2d prefictLeft;
        if (conesEdgeRight.size() < 3)
            return conesEdgeLeft;
        ...
        {
            Point2d startIpm = ipm.homography(Point2d(conesEdgeRight[0].y, conesEdgeRight[0].x)); // 透视变换
        ...
        // Middle
        Point2d middleIpm = ipm.homography(Point2d(conesEdgeRight[conesEdgeRight.size() / 2].y, conesEdgeRight[conesEdgeRight.size() / 2].x)); // 透视变换
        offset = 180;
        ...
        // End
        Point2d endIpm = ipm.homography(Point2d(conesEdgeRight[conesEdgeRight.size() - 1].y, conesEdgeRight[conesEdgeRight.size() - 1].x)); // 透视变换
        offset =180;
        ...
        for (int i = 0; i < repair.size(); i++)
        {
            ...
            conesEdgeLeft.push_back(repair[i]);
        }
