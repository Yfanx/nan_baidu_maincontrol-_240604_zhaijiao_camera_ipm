...
bool cameraCalibration(string imagesFolder)
{
 ...
 sleep(3);
    ...
    cout << "--.999999.../biaoding wancheng --\n\n "<< "\n";
 
   
  
    if (pointsCorners.size() < 1)
    { cout << "VVVVV--.xxxxxxxxx --\n\n "<< "\n";
     sleep(10);
        cout << "Can't find any images!!" << endl;
        return false;
    }
  ...
  // cout << "已筛选 " << pointsCorners.size() << "张合格照片进行标定 | Loading...." << endl;
     //[4] 开始标定图像
    Size sizeSquare = Size(25, 25);       // 实际测量得到的标定板上每个棋盘格的大小：单位mm
 ...
 cout << "save....../res/calibration/assessment.txt\n\n "<< "\n";
...
//   
  fout << "fenbianlv: " << sizeImage.width << "x" << sizeImage.height << "\n";

//     cout << "每幅图像的标定误差: \n";
  
 //  fout << "每幅图像的标定误差: \n";

  cout << "BBBBBBB--..00000.../biaoding wancheng --\n\n "<< "\n";
 ...
 //sleep(16);
   ...
   cout << "--AAAAA.../biaoding wancheng --\n\n "<< "\n";
 //
//sleep(35);  
 for (int i = 0; i < pointsCorners.size(); i++)
    {
        string filename = getFilename(imagesPath[i]);
        fout << "pic[" << filename << "]rot: " << endl;
        fout << rvecsMat[i] << endl;
        //将旋转向量转换为相对应的旋转矩阵
        Rodrigues(rvecsMat[i], rotation_matrix);
        fout << "pic[" << filename << "]rot: " << endl;
        fout << rotation_matrix << endl;
        fout << "pic[" << filename << "]translate: " << endl;
